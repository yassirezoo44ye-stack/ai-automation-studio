    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {!loading && agents.length === 0 && (
        <div className="empty-state" style={{ padding: "60px 24px" }}>
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" style={{ color: "#2E2E2E", marginBottom: 12 }}>
            <path d="M12 2a5 5 0 0 1 5 5v2a5 5 0 0 1-10 0V7a5 5 0 0 1 5-5z"/>
            <path d="M2 20c0-3 3.5-5 10-5s10 2 10 5"/>
          </svg>
          <div style={{ fontWeight: 600, color: "#DDDDDD", marginBottom: 6 }}>No agents yet</div>
          <div style={{ color: "#8A8A8A", fontSize: 13, marginBottom: 16 }}>Create your first agent or start from a template below</div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 12 }}>
            {agents.map(a => {
              ...
              const modelColor = a.model?.includes("opus") ? "#FFD700" : a.model?.includes("haiku") ? "#00C853" : "#E8C87D";
              return (
                <div key={a.id} style={{ ...S.card, cursor: "default", position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "center", flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {a.description && <div style={{ fontSize: 12, color: "#9E9E9E", marginTop: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.description}</div>}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 2, flexShrink: 0 }}>
                      ...
                      <button onClick={() => void deleteAgent(a.id, a.name)} className="btn-icon" title="Delete" style={{ width: 30, height: 30, color: "#FF5252" }}>
                  ...
                  <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid #242424", display: "flex", gap: 16, alignItems: "center" }}>
                    <span style={{ fontSize: 11, color: "#8A8A8A" }}>{a.message_count ?? 0} messages</span>
                    <span style={{ fontSize: 11, color: "#8A8A8A" }}>{relTime(a.created_at)}</span>
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5 }}>
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#00C853" }} />
                      <span style={{ fontSize: 11, color: "#00C853" }}>Ready</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 12 }}>
          {AGENT_TEMPLATES.map(t => (
            <div
              ...
              onMouseEnter={e => (e.currentTarget.style.borderColor = "#E8C87D")}
              onMouseLeave={e => (e.currentTarget.style.borderColor = "")}
            >
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: "#181818", border: "1px solid #2E2E2E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{t.avatar}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ ...S.cardTitle, marginBottom: 4 }}>{t.name}</div>
                  <div style={{ fontSize: 12, color: "#9E9E9E", lineHeight: 1.5 }}>{t.description}</div>
                </div>
              </div>
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid #242424", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#8A8A8A" }}>Template</span>
                <span style={{ fontSize: 11, color: "#E8C87D", display: "flex", alignItems: "center", gap: 4 }}>
                  Use template
