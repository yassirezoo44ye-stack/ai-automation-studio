...
const s: Record<string, React.CSSProperties> = {
  ...
  secTitle:  { fontSize: "11px", fontWeight: 600, color: "#8F8F8F", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "8px", display: "flex", alignItems: "center", justifyContent: "space-between" },
  addBtn:    { fontSize: "18px", lineHeight: 1, background: "none", border: "none", color: "#D4AF37", cursor: "pointer", padding: "0 4px" },
  colorGrid: { display: "flex", flexWrap: "wrap" as const, gap: "6px" },
  swatch:    { width: "32px", height: "32px", borderRadius: "6px", border: "1px solid #2A2A2A", cursor: "pointer", position: "relative" as const, flexShrink: 0 },
  swatchDel: { position: "absolute" as const, top: "-4px", right: "-4px", width: "14px", height: "14px", borderRadius: "50%", background: "#FF5252", color: "#fff", fontSize: "9px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", border: "none", lineHeight: 1 },
  fontItem:  { padding: "6px 8px", borderRadius: "6px", background: "#1A1A1A", marginBottom: "4px", display: "flex", alignItems: "center", justifyContent: "space-between" },
  fontName:  { fontSize: "13px", color: "#F2F2F2" },
  fontSub:   { fontSize: "10px", color: "#BDBDBD" },
  logoItem:  { display: "flex", alignItems: "center", gap: "8px", padding: "4px 0", borderBottom: "1px solid #1A1A1A" },
  logoThumb: { width: "40px", height: "28px", background: "#1A1A1A", borderRadius: "4px", border: "1px solid #2A2A2A", objectFit: "contain" as const },
  delBtn:    { background: "none", border: "none", color: "#8F8F8F", cursor: "pointer", fontSize: "14px", lineHeight: 1 },
  emptyNote: { color: "#8F8F8F", fontSize: "12px", textAlign: "center" as const, padding: "8px 0", fontStyle: "italic" as const },
  addColorBtn:{ padding: "0", width: "32px", height: "32px", borderRadius: "6px", border: "2px dashed #2A2A2A", cursor: "pointer", background: "transparent", color: "#8F8F8F", fontSize: "18px", display: "flex", alignItems: "center", justifyContent: "center" },
};
        ...
        <div style={s.colorGrid}>
          ...
          {addingColor ? (
            <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
              ...
              />
              <button
                style={{ padding: "4px 8px", fontSize: "12px", background: "#D4AF37", color: "#0a0a0a", border: "none", borderRadius: "4px", cursor: "pointer" }}
                onClick={addColor}
              >Add</button>
            </div>
          ) : (
      ...
      <div style={s.section}>
        ...
        {kit.logos.map((l: BrandLogo) => (
          <div key={l.id} style={s.logoItem}>
            ...
            <span style={{ flex: 1, fontSize: "12px", color: "#D6D6D6", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.name}</span>
