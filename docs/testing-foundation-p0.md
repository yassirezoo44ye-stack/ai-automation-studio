...
- **Closed** (commit `34f081c`): RLS coverage for the remaining tables in `_RLS_TABLES` — `billing_events`, `sandbox_workers`, `sandbox_events`, `chat_messages`, `teams`, `usage_records`, `marketplace_installs`, `api_keys`, `marketplace_downloads`, `plugin_installations` — all now have real-Postgres tests (`tests/db_integration/test_rls_teams_chat.py`, `test_rls_marketplace.py`, `test_rls_sandbox_plugins.py`, `test_rls_billing_usage.py`, `test_rls_api_keys.py`). Every table in `_RLS_TABLES` now has direct real-Postgres coverage.
