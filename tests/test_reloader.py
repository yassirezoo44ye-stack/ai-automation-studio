...
registry with no rollback and self._ownership left stale.

Real temp .py files are used (matching tests/test_plugins.py's
TestPluginLoaderIsolation convention) because the bug is specifically about
exec_module()/spec.loader failure modes that a mock can't faithfully
reproduce. They're created under the project root (not the OS temp dir)
because _module_name() requires the path to be relative to _PROJECT_ROOT.
"""
