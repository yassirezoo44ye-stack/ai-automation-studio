"""
Discord — AUTH_PROVIDER example plugin. Demonstrates the Plugin SDK's
OAuth2 identity-provider extension point end to end, using Discord's real,
publicly documented OAuth2 endpoints and the exact authorization-code-flow
...
exactly discord.com. configuration_schema requires client_id/
client_secret/redirect_uri, supplied by an org admin via
PUT /plugins/installed/{id}/config after installing (a Discord
application must be registered first to obtain these). client_secret
...
_HEADERS = {"Accept": "application/json", "User-Agent": "axon-plugin-sdk/discord"}
...
class DiscordAuthProvider:
    ...
    async def exchange_code(self, *, code: str, redirect_uri: str) -> dict:
        ...
        # Discord's newer "global display name" supersedes the legacy
        # username#discriminator pair when present.
...
class DiscordPlugin(PluginBase):
    ...
    def register(self, ctx: PluginContext) -> None:
        ...
        adapt_auth_provider("discord", DiscordAuthProvider(ctx.config))

    def unregister(self, ctx: PluginContext) -> None:
        from app.plugins.adapters import unadapt_auth_provider
        unadapt_auth_provider("discord")

    def health_check(self) -> PluginHealth:
        return PluginHealth(plugin_id="discord", state=PluginState.ENABLED)
