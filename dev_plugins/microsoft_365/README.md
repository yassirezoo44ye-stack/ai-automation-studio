...
Microsoft" using the platform's existing Plugin SDK + Sandbox + OAuth
...
1. Register an app in [Azure Portal -> App registrations](https://portal.azure.com/), add a client secret, and add your redirect URI under "Web" platform.
...
5. The registered `microsoft_365` AUTH_PROVIDER is now callable via `app.plugins.adapters.WorkerProxyProvider` — `get_authorization_url(redirect_uri, state)` builds the consent-screen URL, `exchange_code(code, redirect_uri)` completes the flow and returns a normalized `{email, name}` profile (fetched from Microsoft Graph's `/me`).
