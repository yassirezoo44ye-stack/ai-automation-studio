"""
Groq provider — fast LPU-backed inference over an OpenAI-compatible API.

Set GROQ_API_KEY to enable. Optionally set GROQ_DEFAULT_MODEL (defaults to
...
# USD per token (catalog.py carries the authoritative per-million figures —
# this mirrors them so calculate_cost() works even for a model id the
...
class GroqProvider(BaseProvider):
    ...
    # _env_key() default (f"{provider_id.upper()}_API_KEY" = "GROQ_API_KEY")
    # is already correct — no override needed.

    def default_model(self) -> str:
        import os
        return os.getenv("GROQ_DEFAULT_MODEL", _DEFAULT_MODEL)
